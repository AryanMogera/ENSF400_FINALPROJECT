...
### Gradle Build Error: Missing commit.html

If you encounter a build error related to `src\main\webapp\commit.html` with a message like:
```
Caused by: java.io.FileNotFoundException: src\main\webapp\commit.html (The system cannot find the path specified)
```

#### Solution:

This file is automatically generated by the `writeGitCommitToFile` Gradle task but needs to exist before certain build tasks run. To fix this issue:

1. Run the file generation task first:
   ```
   gradlew writeGitCommitToFile
   ```

2. Then run your regular build command:
   ```
   gradlew build or gradlew apprun (These commands should automatically run the task mentioned above so its not required to run the above command if gradlew apprun is being used)
   ```

**Note:** Do not remove this file from `.gitignore` as it's intended to be generated during build time with your specific commit hash.
